...
## DX Machine Performance Win

This fork has a proof-backed DX serializer win on the current JSON-heavy G-drive benchmark. With pre-generated `.machine` artifacts prepared before timing, the local DX cache-enabled build beat official `rolldown@1.0.3` on both the normal run and the slow-end run, while producing matching output hashes across every arm.

| Arm | Median | p95 |
| --- | ---: | ---: |
| Official `rolldown@1.0.3` | `219.71 ms` | `467.14 ms` |
| Local cache disabled | `218.37 ms` | `422.23 ms` |
| Local DX cache enabled | `208.92 ms` | `358.88 ms` |

The headline is the p95 result: the slow-end timing improved by about `30.17%` versus official, dropping from `467.14 ms` to `358.88 ms`. Median timing also improved by `5.17%`, from `219.71 ms` to `208.92 ms`. This run used `18` JSON files, `1280` rows per file, `30` measured samples, and `5` warmups; `.machine` generation was not counted inside the timed benchmark window.

Scope: this is a measured benchmark snapshot for the current JSON/config-heavy DX workload, not a universal claim for every Rolldown project. The proof log is stored at `G:\Dx\rolldown-benchmarks\run-20260531-current-governed\logs\current-governed-official-vs-local-18x1280-now-30samples.log`.
