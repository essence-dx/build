...
impl FileToUrlEnv<'_> {
  ...
  pub async fn file_to_built_url(
    ...
    let should_inline = self.should_inline(file, &id, &content, force_inline).await?;
    if let Some(cached) = cache.0.get(id.as_ref()) {
      return Ok(cached.to_string());
    }

    let url = if should_inline {
...
fn svg_to_data_url(content: &[u8]) -> String {
    ...
    || NESTED_QUOTES_RE.is_match(&string_content)
  {
    ...
    // Replace characters - the one-pass encoder preserves the old "% first" behavior.
    result = encode_svg_special_chars(&result);
...
fn encode_svg_special_chars(value: &str) -> String {
  let mut output = String::with_capacity(svg_special_encoded_len(value));
  let mut chunk_start = 0;
  for (index, byte) in value.as_bytes().iter().copied().enumerate() {
    let Some(encoded) = svg_special_char_encoding(byte) else {
      continue;
    };
...
fn svg_special_encoded_len(value: &str) -> usize {
  value.len()
    + value.as_bytes().iter().filter(|byte| matches!(byte, b'%' | b'#' | b'<' | b'>')).count() * 2
}

fn svg_special_char_encoding(byte: u8) -> Option<&'static str> {
  match byte {
    b'%' => Some("%25"),
    b'#' => Some("%23"),
    b'<' => Some("%3c"),
...
mod tests {
  ...
  #[test]
  fn svg_special_char_encoding_matches_legacy_replace_order_with_exact_capacity() {
    let input = r##"%#<svg viewBox="0 0 1 1">%23"##;
    let encoded = encode_svg_special_chars(input);

    assert_eq!(encoded, "%25%23%3csvg viewBox='0 0 1 1'%3e%2523");
    assert_eq!(encoded.capacity(), encoded.len());
  }

  #[test]
  fn svg_special_char_encoding_preserves_unicode_chunks() {
    let input = "雪%\"#<>";
    let encoded = encode_svg_special_chars(input);

    assert_eq!(encoded, "雪%25'%23%3c%3e");
    assert_eq!(encoded.capacity(), encoded.len());
  }

  #[test]
  fn svg_to_data_url_preserves_uri_output_shape_after_one_pass_encoding() {
    let content = br##"<svg>
      <path fill="#fff" d="M0 0"/>
    </svg>"##;

    assert_eq!(
      svg_to_data_url(content),
      "data:image/svg+xml,%3csvg%3e%3cpath%20fill='%23fff'%20d='M0%200'/%3e%3c/svg%3e"
    );
  }

  #[test]
  fn svg_to_data_url_keeps_base64_fallback_for_text_content() {
    let content = br#"<svg><text>hello</text></svg>"#;

    assert_eq!(
      svg_to_data_url(content),
      concat_string!("data:image/svg+xml;base64,", to_standard_base64(content))
    );
  }

  #[test]
  fn file_to_built_url_rechecks_asset_cache_after_async_read() {
    let source =
      std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/src/file_to_url.rs")).unwrap();
    let method_source = source.split("  async fn should_inline").next().unwrap();
    let read_index = method_source.find("let content = tokio::fs::read(file).await?;").unwrap();
    let inline_index = method_source
      .find("let should_inline = self.should_inline(file, &id, &content, force_inline).await?;")
      .unwrap();

    assert!(
      method_source[read_index..inline_index]
        .contains("if let Some(cached) = cache.0.get(id.as_ref())"),
      "asset URL creation should recheck AssetCache after async file IO before inline/emission work"
    );
  }

  #[test]
  fn file_to_built_url_rechecks_asset_cache_after_inline_decision() {
    let source =
      std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/src/file_to_url.rs")).unwrap();
    let method_source = source.split("  async fn should_inline").next().unwrap();
    let inline_index = method_source
      .find("let should_inline = self.should_inline(file, &id, &content, force_inline).await?;")
      .expect("asset URL creation should name the async inline decision");
    let data_url_index =
      method_source.find("self.asset_to_data_url(file.as_path(), &content)?").unwrap();
    let emit_index = method_source.find("self.ctx.emit_file_async(emitted_asset).await?").unwrap();

    assert!(
      method_source[inline_index..data_url_index]
        .contains("if let Some(cached) = cache.0.get(id.as_ref())"),
      "asset URL creation should recheck AssetCache after async inline decision before data URL work"
    );
    assert!(
      method_source[inline_index..emit_index]
        .contains("if let Some(cached) = cache.0.get(id.as_ref())"),
      "asset URL creation should recheck AssetCache after async inline decision before emit work"
    );
  }
}
