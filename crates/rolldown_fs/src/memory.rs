...
mod tests {
  ...
  #[test]
  fn memory_file_system_reports_inline_source_reads() {
    let fs = MemoryFileSystem::default();

    assert!(
      !fs.should_spawn_blocking_reads(),
      "memory-backed reads are already in-process and should not pay blocking-pool dispatch"
    );
  }
}
