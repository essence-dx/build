...
impl Plugin for ViteResolvePlugin {
  ...
  async fn watch_change(
    ...
    if watch_change_should_clear_resolver_cache(event) {
      self.resolvers.clear_cache();
    }
    Ok(())
  }
...
fn watch_change_should_clear_resolver_cache(event: WatcherChangeKind) -> bool {
  matches!(event, WatcherChangeKind::Create | WatcherChangeKind::Delete | WatcherChangeKind::Update)
}
...
mod tests {
  ...
  #[test]
  fn watch_change_update_clears_resolver_cache() {
    assert!(watch_change_should_clear_resolver_cache(WatcherChangeKind::Update));
  }
}
